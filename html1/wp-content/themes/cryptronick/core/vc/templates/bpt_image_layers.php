<?php
	...
	$compile = $img_layer_image = $img_layer_wrap_classes = $img_layer_classes = $offset_styles = $styles = $link_attr = '';
	...
	$img_layer_id = uniqid( "cryptronick_img_layer_" );
	$img_layer_id_attr = 'id='.$img_layer_id;

	// image layer wrapper classes
    $img_layer_wrap_classes .= !empty($item_el_class) ? ' '.$item_el_class : '';
        ...
        $img_layer_image .= '<div class="img_layer_image_wrapper '.esc_attr($item_d['image_animation']).'" style="z-index:'.esc_attr($item_d['image_order']).'">';
	        $img_layer_image .= '<div class="img_layer_item" '.$offset_styles.'>';
	       		$img_layer_image .= '<div class="img_layer_image">';
	       			$img_layer_image .= '<img src="'.esc_url($featured_image[0]).'" alt="'.(!empty($img_alt) ? $img_alt : '').'" />';
	        	$img_layer_image .= '</div>';
	        $img_layer_image .= '</div>';
	    $img_layer_image .= '</div>';
    }

	$compile .= '<div '.esc_attr($img_layer_id_attr).' class="cryptronick_module_img_layer'.esc_attr($img_layer_wrap_classes).'">';

		$compile .= !empty($url) ? "<a ".$link_attr.">" : '';
		$compile .= $img_layer_image;
		$compile .= !empty($url) ? '</a>' : '';
