<?php
	...
	$compile = $message_id = $message_id_attr = $message_inner = $animation_class = $message_wrap_classes = $fontawesome = '';

	// adding uniq id for message box module
	if ($type == 'custom') {
		$message_id = uniqid( "cryptronick_message_" );
	...
	extract( $obj->getAttributes( $atts, $this, $this->shortcode, array('google_fonts_title', 'google_fonts_text') ) );
	...
	// render html
	$compile .= '<div '.esc_attr($message_id_attr).' class="cryptronick_module_message_box'.esc_attr($message_wrap_classes).'">';
		$compile .= $message_inner;
	$compile .= '</div>';
