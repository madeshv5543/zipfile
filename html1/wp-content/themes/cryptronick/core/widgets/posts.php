...
class posts extends WP_Widget
{
    ...
    function __construct() {
        parent::__construct(false, esc_html__('Posts (current theme)', 'cryptronick') );
    }
    ...
    function widget($args, $instance) {
        ...
        echo Cryptronick_Theme_Helper::render_html($before_widget);
        echo Cryptronick_Theme_Helper::render_html($before_title);
        echo esc_attr($instance['widget_title']);
        echo Cryptronick_Theme_Helper::render_html($after_title);
        ...
        $cryptronick_wp_query_posts = new WP_Query();
        $cryptronick_wp_query_posts->query($postsArgs);
        $counter = 1;
        while ($cryptronick_wp_query_posts->have_posts()) : $cryptronick_wp_query_posts->the_post();
            $cryptronick_theme_featured_image_latest = wp_get_attachment_image_src(get_post_thumbnail_id(get_the_ID()));

            $compilepopular .= '
            <li ' . ((!empty($cryptronick_theme_featured_image_latest)) ? 'class="with_img"' : '') . '>';
            if (empty($cryptronick_theme_featured_image_latest)) {
                $widg_img = '';
            } else {
                $widg_img = '<a href="' . esc_url(get_permalink()) . '"><img src="' . esc_url(aq_resize($cryptronick_theme_featured_image_latest[0], "170", "170", true, true, true)) . '" alt="' . get_the_title() . '"></a>';
            }
            ...
            if (!empty($instance['posts_widget_content_hide']) && $instance['posts_widget_content_hide'] == 'on') {
                ...
                $post_descr = Cryptronick_Theme_Helper::modifier_character($post_excerpt_without_tags, 50, "...");
            }else{
                $post_descr = '';
			...
			$compilepopular .= '
                <div class="recent_posts_content'.esc_attr($widget_class).'">
                    <span class="counter_posts">'.$counter++.'</span>
                    ' . $widg_img . '
        ...
        echo Cryptronick_Theme_Helper::render_html($after_widget);
    }
    ...
    function form($instance) {
        $defaultValues = array(
            'widget_title' => esc_html__( 'Recent Posts', 'cryptronick' ),
        ...
        <table class="fullwidth">
            <tr>
                <td><?php echo esc_html__( 'Title:', 'cryptronick' ); ?></td>
            ...
            <tr>
                <td><?php echo esc_html__( 'Number:', 'cryptronick' ); ?></td>
            ...
            <tr>
                <td><?php echo esc_html__( 'Show post content:', 'cryptronick' ); ?></td>
