...
function metabox_tab(){
	...
	jQuery( '#advanced-sortables' ).wrapInner( "<div class='cryptronick_tab_meta_container'></div>" );
	jQuery( '#advanced-sortables' ).append('<div class="cryptronick_tab_meta_titles"></div>')
	jQuery('#advanced-sortables').find('.postbox ').each(function(){
		title = jQuery(this).children('h2').children('span').clone();
		id = jQuery(this).attr('id');
		jQuery('#advanced-sortables').find('.cryptronick_tab_meta_titles').append('<h2 class="'+id+'">'+(title.text())+'</h2>');
	})

	jQuery('.cryptronick_tab_meta_container').children().hide();
	jQuery('.cryptronick_tab_meta_container').find('.postbox:first-child').show();
	jQuery('.cryptronick_tab_meta_titles h2:first-child').addClass('active');
	jQuery('#advanced-sortables').find('.cryptronick_tab_meta_titles h2').on("click",function(){
