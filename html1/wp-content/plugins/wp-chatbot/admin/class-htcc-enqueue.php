...
class HTCC_Enqueue {
    ...
    function enqueue( $hook ) {
        ...
        if( 'toplevel_page_wp-chatbot' == $hook || 'wp-chatbot_page_wp-chatbot-features' == $hook || 'wp-chatbot_page_wp-chatbot-actions' == $hook || 'wp-chatbot_page_wp-chatbot-pro-woo' == $hook ) {
            ...
            wp_enqueue_script( 'htcc_admin_color_picker_js', 'https://cdnjs.cloudflare.com/ajax/libs/spectrum/1.8.0/spectrum.min.js', array( 'jquery', 'wp-color-picker', 'htcc_admin_md_js' ), HTCC_VERSION, true );
            ...
            wp_enqueue_script( 'htcc_admin_js', plugins_url( 'admin/assets/js/admin.js', HTCC_PLUGIN_FILE ), array( 'wp-color-picker', 'jquery', 'htcc_admin_md_js' ), HTCC_VERSION, true );
