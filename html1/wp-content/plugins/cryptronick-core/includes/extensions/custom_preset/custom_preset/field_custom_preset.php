...
if( !class_exists( 'ReduxFramework_custom_preset' ) ) {
    ...
    class ReduxFramework_custom_preset{
        ...
        public function render() {
            ...
            ?>
            <div id="bpt_custom_preset">
            <div id="redux-add-preset-action"><input type="submit" id="redux-add-preset" value="<?php esc_html_e( 'Add Preset', 'redux-framework' ) ?>"><i class="fa fa-plus"></i></div>
            ...
            <div id="redux-save-preset-action" style="display:none;">
                <div class="wrap-save-preset">
                    ...
                    <input type="submit" id="redux-save-preset" class="button-primary" value="<?php esc_html_e( 'Save', 'redux-framework' ) ?>">                    
                </div>
            ...
            if ( isset( $this->field['options'] ) ) {
                if(!empty($this->field['options'])){
                    echo "<h4>".__('Saved Templates', 'cryptronick-core')."</h4>";
                }
                ...
                foreach ( $this->field['options'] as $k => $v ) {
                    ...
                    echo '<input type="radio" class="' . $this->field['class'] . '" id="' . $this->field['id'] . '_' . ( array_search( $k, array_keys( $this->field['options'] ) ) + 1 ) . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" value="' . $theValue . '" ' . checked( $this->value, $theValue, false ) . $presets . '/>';
                        ...
                        echo '<span class="' . $v['class'] . '">' . $v['alt'] . '</span>';

                    echo '</label>';
                    echo '<div class="wrapper-btn-preset">';
                        if ( $selected ) { 
                            echo '<div id="selected-btn-preset" class="btn-preset">';                                                  
                                echo '<input type="submit" class="selected-btn_custom btn-primary redux-update-preset" value="'.apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Save Custom Header', 'cryptronick-core' ) ).'">'; 
                        ...
                        if(!$v['default'] && !array_key_exists($k, $list_defualt_preset)){
                             echo '<div class="delete-preset">';
                                echo '<a href="#" title="'.apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Delete Preset', 'cryptronick-core' ) ).'" class="redux-delete-preset">';
                        ...
                        if(!$v['default'] && array_key_exists($k, $list_defualt_preset)){
                             echo '<div class="delete-preset">';
                                echo '<a href="#" title="'.apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", __( 'Reset', 'cryptronick-core' ) ).'" class="redux-delete-preset reset-default">';
        ...
        public function enqueue() {
            ...
            $vars = array(
                'delete'   => __( 'Delete Preset?', 'cryptronick-core' ),
                'reset'   => __( 'Reset to default?', 'cryptronick-core' ),
                'taken_name'   => __( 'Sorry this name is already exists', 'cryptronick-core' ),
                'empty_name'   => __( 'Empty name', 'cryptronick-core' ),
            );

            wp_localize_script( 'redux-field-icon-custom_preset-js', 'bptLocalVars', $vars );
